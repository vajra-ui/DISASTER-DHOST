...
export const CommanderDashboard: React.FC = () => {
  ...
  const [filter, setFilter] = useState<'ALL' | 'CLUSTERS' | 'CRITICAL' | 'FLOOD' | 'MEDICAL' | 'UNASSIGNED'>('ALL');
  ...
  const handleOpenXai = (inc: EmergencyPacket) => {
    setXaiPacket(inc);
    setIsXaiOpen(true);
  };
  ...
  return (
    <div className="min-h-[calc(100vh-56px)] bg-slate-950 text-slate-100 p-4 md:p-6 max-w-[1600px] mx-auto space-y-4 animate-in fade-in select-none">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-4">
        ...
        {/* ======================================================== */}
        {/* COLUMN 1: LEFT SIDEBAR (AI RADAR & CLUSTER STATS) [3 COLS]*/}
        {/* ======================================================== */}
          ...
          <div className="p-4 rounded-3xl bg-slate-900 border border-slate-800 space-y-3 shadow-lg">
            ...
            <div className="grid grid-cols-2 gap-2">
              ...
              <div 
                ...
                className={`p-3 rounded-2xl border cursor-pointer transition ${
                  filter === 'CLUSTERS' ? 'bg-purple-500/20 border-purple-500' : 'bg-slate-950 border-slate-800 hover:border-slate-700'
                }`}
              >
                <span className="text-[10px] font-bold text-purple-400 block uppercase">🔁 Spatial Clusters</span>
        ...
        <div className="lg:col-span-5 space-y-3">
          ...
          {/* SPATIAL CLUSTERS VIEW (Deduplication) */}
          {filter === 'CLUSTERS' ? (
            <div className="space-y-3">
              <div className="p-3 rounded-2xl bg-purple-950/40 border border-purple-500/50 text-xs text-purple-200">
                <span className="font-black">🔁 Spatial Clustering Active:</span> Multiple individual distress reports within 250m radius have been grouped to prevent duplicated responder dispatches.
              </div>

              {clusters.map(cluster => (
                <div 
                  key={cluster.clusterId}
                  className="p-4 rounded-3xl bg-slate-900 border-2 border-purple-500/50 space-y-3 shadow-lg"
                >
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      <span className="font-mono text-xs font-black text-purple-400 bg-purple-500/20 px-2 py-0.5 rounded border border-purple-500/40">
                        {cluster.clusterId}
                      </span>
                      <span className="font-black text-white text-xs">{cluster.clusterName}</span>
                    </div>
                    <span className="px-2 py-0.5 rounded bg-red-500/20 text-red-400 font-bold text-[10px]">
                      {cluster.consolidatedPriority}
                    </span>
                  </div>

                  <div className="grid grid-cols-2 gap-2 text-xs p-3 rounded-2xl bg-slate-950 border border-slate-800 font-mono">
                    <div>
                      <span className="text-slate-500 text-[10px] block">TOTAL REPORTS</span>
                      <span className="font-bold text-white text-base">{cluster.totalIncidents} Distress Beacons</span>
                    </div>
                    <div>
                      <span className="text-slate-500 text-[10px] block">TOTAL CASUALTIES</span>
                      <span className="font-bold text-amber-400 text-base">{cluster.totalPeople} Individuals</span>
                    </div>
                  </div>

                  <div className="flex items-center justify-between text-[11px] text-slate-400">
                    <span>📍 Cluster Landmark: {cluster.landmark}</span>
                    <span className="text-purple-300 font-mono">Radius: {cluster.radiusMeters}m</span>
                  </div>
                </div>
              ))}
            </div>
                      ...
                      <div className="flex items-center justify-between gap-2">
                        ...
                        <div className="flex items-center gap-1.5">
                          <button
                            ...
                            className="px-2.5 py-1 rounded-full bg-amber-500/20 hover:bg-amber-500/30 border border-amber-500/40 text-amber-300 text-[10px] font-mono font-black flex items-center gap-1 transition"
                            title="Why Critical? Explainable AI Inspector"
                          >
                            <Brain className="w-3 h-3 text-amber-400" />
                            <span>Why {inc.priority}? ({analysis.urgencyScore}%)</span>
                          </button>
                        </div>
                      </div>
